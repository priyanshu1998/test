    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)//call for construction of base class members
{
    ...
    connect(ui->pushButton,SIGNAL(clicked()),this,SLOT(simple_ellipse_click()));
    connect(ui->pushButton_1,SIGNAL(clicked()),this,SLOT(ellipse_midpoint_click()));
...
void MainWindow::ellipse_midpoint_click(){
    obj.rx = ui->radius_x->text().toInt();
    obj.ry = ui->radius_y->text().toInt();

    std::vector<point> onCurve = obj.midpt_Ellip_draw();
    display_points(onCurve);
    onCurve.clear();
}

void MainWindow::simple_ellipse_click(){
    obj.rx = ui->radius_x->text().toInt();
    obj.ry = ui->radius_y->text().toInt();

    std::vector<point> onCurve = obj.simpEllip_draw();
    display_points(onCurve);
    onCurve.clear();
